...
class CDAATSampler(torch.utils.data.Dataset):
    """
            ...
            if is_video_dataset:
                ...
                while search_frame_ids is None:
                    ...
                    if prev_frame_ids is None:
                        gap_add += 5
                        continue
                    selected_frame_ids = base_frame_id # + prev_frame_ids  # one template
                ...
                search_frame_ids = [1]
                reference_frame_ids = []
                for _ in range(num_short_refer):        # one template for _ in range(num_short_refer + 1):
                    reference_frame_ids.append(1)

            template_frames, template_depths, template_anno, meta_obj_template = dataset.get_frames(seq_id, reference_frame_ids, seq_info_dict)
            search_frames,  search_depths, search_anno, meta_obj_search = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict)

            data = TensorDict({'reference_images': template_frames,
                               'reference_depths': template_depths,
                               'reference_anno': template_anno['bbox'],
...
class BASELINESampler(torch.utils.data.Dataset):
    """
    Class responsible for sampling frames from training sequences to form batches.
