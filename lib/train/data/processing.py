...
class CDAATRACKProcessing(BaseProcessing):
    """
    ...
    def __call__(self, data: TensorDict):
        ...
        for s in ['search', 'reference']:
            ...
            crops_depth, _, _ = prutils.jittered_center_crop(data[s + '_depths'], jittered_anno, data[s + '_anno'],
                                                             self.search_area_factor[s], self.output_sz[s])

            # original image patch for color-aware attention
            data[s + '_orimages'] = [torch.tensor(np.array(crop_patch, dtype=np.float32)).squeeze() for crop_patch in
                                     crops]
            # depth crop
            data[s + '_depths'] = [torch.tensor(np.array(crop_depth, dtype=np.float32)).squeeze() for crop_depth in
                                   crops_depth]
...
class BASELINEProcessing(BaseProcessing):
    """
    The processing class used for training LittleBoy. The images are processed in the following way.
