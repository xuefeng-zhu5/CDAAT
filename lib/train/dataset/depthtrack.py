...
class DepthTrack(BaseVideoDataset):
    ...
    def __init__(self, root=None, dtype='rgbd', split='train',  image_loader=jpeg4py_loader, vid_ids=None): #  split=None, data_fraction=None):
        """
        args:
    ...
    def _get_frame(self, seq_path, frame_id):
        ...
        if self.dtype =='color':
            return get_rgbd_frame(color_path, depth_path, dtype=self.dtype, depth_clip=True)
        else:
            img, depth = get_rgbd_frame(color_path, depth_path, dtype=self.dtype, depth_clip=True)
            # print(depth.dtype, np.max(depth))
            return img, depth
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        if self.dtype=='color':
            ...
            image_list = [rgbd[0] for rgbd in frame_list]
            depth_list = [rgbd[1] for rgbd in frame_list]
            return image_list, depth_list, anno_frames, object_meta
