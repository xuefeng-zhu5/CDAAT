...
class Tracker:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        # Initialize
        img, depth = self._read_image(seq.frames[0])

        start_time = time.time()
        out = tracker.initialize(img, depth, init_info, seq.name)
        if out is None:
            out = {}
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            img, depth, = self._read_image(frame_path)
            ...
            out = tracker.track(img, depth, info, seq.name)
            ...
            if env.show_result:
                ...
                cv.rectangle(img, [bbox[0], bbox[1]],
                             ...
                             (0, 255, 0), 3)
                cv.putText(img, str(out['confidence']), (40, 40), cv.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
    ...
    def _read_image(self, image_file: str, dtype='rgbrawd'):
        img, depth = get_rgbd_frame(image_file['color'], image_file['depth'], dtype=dtype, depth_clip=True)
        return img, depth
